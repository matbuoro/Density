...
#Heatmap abondance
ggplot(data, aes(year, basin, fill= abundance)) + 
  geom_tile() +
scale_fill_gradient(low = "#ADD8E6", high = "#00008B") +
  theme_minimal()+
  ggtitle("Spatio-temporal distribution of data, all fishing methods")
...
ggplot(data, aes(x=Method, y=area))+
  geom_boxplot()+
...
for (basin in basins) {
 ...
 p1 <- ggplot(subset_data, aes(x=as.factor(month), y=abundance)) + 
   ...
   ggtitle(paste("Abondance par mois", basin))+
   theme_minimal()
 #print(p1)
 ...
 p2 <-  ggplot(subset_data, aes(x=as.factor(year), y=abundance, color=Method)) + 
   ...
   theme_minimal()+
   theme(axis.text.x = element_text(angle = 45, hjust = 1, size=8))+
   ggtitle(paste("Abondance par année et méthode", basin))
 #print(p2)
 
 p3 <-   ggplot(subset_data, aes(x=as.factor(year), y=area)) + 
   geom_boxplot(fill="lightgreen") + 
   labs(x = "Year", y = "Area (m^2)") + 
   ggtitle(paste("Aire par année", basin))+
   theme_minimal()+
   theme(axis.text.x = element_text(angle = 45, hjust = 1, size=8))
   #print(p3)
