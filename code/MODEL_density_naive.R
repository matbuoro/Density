modelstat<-function(){
 ...
 ## DE LURY
  ...
  for (j in n2[1]:n2[2]){ #y boucle sur les lignes du dataframe
    ...
    log_muP2[j] <- delta[1] + epsilonP[riverID[j]]
  }#end boucle
  ...
  for (j in n3[1]:n3[2]){ #y boucle sur les lignes du dataframe
    ...
    log_muP3[j] <- delta[1] + epsilonP[riverID[j]]# /!\ we consider year effect instead of age because protocol and sampling effort could have change over time.
  ...
  for (pop in 1:max(riverID)){   
    ...
    P_pred[pop]<- ilogit(delta[1]+ epsilonP[pop])
