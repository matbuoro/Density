...
data2$site<-as.character(substr(data2$XYZ, 6, nchar(data2$XYZ)))
...
modelstat<-function()
{
  ...
  for (y in 1:n){ #y boucle sur les lignes du dataframe
    ...
    log_muD[y] <- gamma[1]+ (pow(year[y],gamma[2]))
    #p[y]~dbeta(2,2)
  ...
  for (t in 1:max_year){
    ...
    muD[t]<- exp(gamma[1]+ (pow(t,gamma[2])))
...
traplot(jagsfit, parms = c("alpha","beta","gamma","delta")) #distrib posteriori marginales
denplot(jagsfit, parms = c("alpha","beta","gamma","delta")) #distrib posteriori marginales
